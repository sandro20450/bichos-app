...
/* Estilo para os cards de previsão da IA */
...
.previsao-num { font-size: 32px; font-weight: bold; color: #4CAF50; }
.previsao-chance { font-size: 14px; color: #aaa; }
</style>
...
# Converte Milhar para Int (para IA)
def get_grupo_int(m):
    try:
        d = int(str(m)[-2:])
...
# Converte Milhar para String formatada (para a parte Estatística Tradicional)
def get_grupo_str(m):
    try:
        d = int(str(m)[-2:])
...
# =============================================================================
# --- 5. TELA 2: ESTATÍSTICA TRADICIONAL (SEU ALGORITMO ORIGINAL) ---
# =============================================================================
                ...
                if sh:
                    ...
                    if len(dados_brutos) < 2: st.error("Dados insuficientes.")
                        ...
                        if v_max_c > 0 and v_atual_c >= (v_max_c - 1) and v_atual_c > 0:
                            st.markdown(f"""<div class="gatilho-ativo">🚀 GATILHO TÁTICO ATIVADO! A Zebra encostou no limite histórico de {v_max_c}x! OPORTUNIDADE DE ENTRADA NOS GRUPOS PRINCIPAIS!</div>""", unsafe_allow_html=True)
                        else:
                            st.markdown(f"""<div class="gatilho-espera">⏳ Status: Aguardando exaustão da Zebra. (A Zebra saiu {v_atual_c}x seguidas atualmente. Limite: {v_max_c}x).</div>""", unsafe_allow_html=True)
...
# =============================================================================
# --- 6. TELA 3: CÉREBRO IA PENTÁGONO (NOVO XGBOOST) ---
# =============================================================================
elif menu == "🤖 Radar IA Pentágono (XGBoost)":
    st.title("🤖 Radar Preditivo Pentágono (IA)")
    banca_xgb = st.selectbox("Selecione a Banca Alvo para Previsão IA:", list(BANCAS_CONFIG.keys()), key="sel_banca_xgb")
    
    st.info("💡 A Inteligência Artificial (Gradiente Boosting) vasculha as últimas centenas de sorteios desta banca para encontrar padrões matemáticos escondidos que humanos não conseguem ver.")
    
    if st.button("🚀 Ativar Cérebro Pentágono (Treinar IA)", use_container_width=True, type="primary"):
        with st.spinner("Conectando aos servidores do Google e calibrando IA..."):
            try:
                sh = conectar_sheets()
                if sh:
                    ...
                    if len(dados_brutos) < 50:
                        st.error("⚠️ Dados insuficientes. O XGBoost precisa de pelo menos 50 sorteios para aprender.")
                    else:
                        df = pd.DataFrame(dados_brutos)
                        df = df.iloc[:, :7]
                        df.columns = ["Data", "Sorteio", "P1", "P2", "P3", "P4", "P5"]
                        ...
                        # Convertendo P1 para Grupo Inteiro (1 a 25)
                        df['grupo_alvo'] = df['P1'].apply(get_grupo_int)
                        df = df.dropna(subset=['grupo_alvo']).copy()
                        df['grupo_alvo'] = df['grupo_alvo'].astype(int)
                        
                        # Engenharia de Atributos: O que aconteceu nos 3 sorteios anteriores?
                        # Como os dados mais recentes estão na parte de baixo da tabela:
                        df['ant_1'] = df['grupo_alvo'].shift(1)
                        df['ant_2'] = df['grupo_alvo'].shift(2)
                        df['ant_3'] = df['grupo_alvo'].shift(3)
                        
                        # Pegamos os últimos 500 registros para focar no comportamento atual da banca
                        df_treino = df.dropna().tail(500)
                        
                        if len(df_treino) < 20:
                            st.error("Erro após a limpeza dos dados. Tente atualizar a base de extração.")
                        else:
                            X = df_treino[['ant_1', 'ant_2', 'ant_3']]
                            y = df_treino['grupo_alvo']
                            
                            # BLINDAGEM TRADUTOR: Resolve o erro de classes faltantes no XGBoost
                            le = LabelEncoder()
                            ...
                            st.success(f"✅ Motor IA Calibrado! Precisão em dados não vistos: {precisao*100:.2f}%")
                            
                            # Realizando a Previsão para o Futuro
                            # Pegamos os últimos 3 grupos que saíram AGORA para prever o próximo
                            ultimos_3 = df['grupo_alvo'].tail(3).values
                            
                            if len(ultimos_3) == 3:
                                entrada = pd.DataFrame({
                                    'ant_1': [ultimos_3[2]], # Mais recente
                                    'ant_2': [ultimos_3[1]], # Penúltimo
                                    'ant_3': [ultimos_3[0]]  # Antepenúltimo
                                })
                                
                                probabilidades = modelo.predict_proba(entrada)[0]
                                top_3_idx = np.argsort(probabilidades)[::-1][:3]
                                ...
                                for i, idx in enumerate(top_3_idx):
                                    # Traduz o código do XGBoost de volta para o Grupo Real
                                    grupo_real = int(le.inverse_transform([idx])[0])
                                    chance = probabilidades[idx] * 100
                                    nome_bicho = bichos_dict.get(grupo_real, "Desconhecido")
                                    ...
                                    with cols[i]:
                                        ...
                                        <div class="previsao-card">
                                            ...
                                            <div class="previsao-chance">Probabilidade: {chance:.1f}%</div>
                                        </div>
                                        """, unsafe_allow_html=True)
                            else:
