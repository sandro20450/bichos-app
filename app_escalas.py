...
# Dicionários para tradução de datas
...
if not st.session_state.logged_in:
    ...
    if is_admin:
        menu_opcoes.append("📊 Consultar Escalas Mensais")
    ...
    if escolha == "🏠 Quadro de Hoje":
        ...
        if not escalas_hoje:
            ...
            # Formata a data para o padrão militar do grupo
            dia_num = f"{data_filtro.day:02d}"
            mes_txt = MESES_PT[data_filtro.month]
            ano_num = data_filtro.year
            dia_semana = DIAS_PT[data_filtro.weekday()]
            data_wpp = f"{dia_num}{mes_txt}{ano_num} {dia_semana}"
            
            # Constrói o texto do cabeçalho
            texto_wpp = "SDS - PMPE - DGA - DAS\n"
            texto_wpp += "*DIRETORIA DE ASSISTÊNCIA SOCIAL*\n"
            texto_wpp += "Tel.: (81) 3181-1861\n\n"
            texto_wpp += "EFETIVO DE SERVIÇO\n\n"
            texto_wpp += f"📆 *Data:* {data_wpp}\n\n"
            
            # Constrói a lista de serviços e policiais
            for srv, policiais in servicos_do_dia.items():
                lista_nomes = []
                horario_geral = ""
                ...
                for pol_escala in policiais:
                    ...
                    dados_pol = dict_efetivo.get(mat, {})
                    grad = dados_pol.get("Graduacao", "")
                    nome = dados_pol.get("Nome", "Não Encontrado")
                    telefone = str(dados_pol.get("Telefone", ""))
                ...
                # Junta os nomes do mesmo serviço com ' e '
                nomes_agrupados = " e ".join(lista_nomes)
                texto_wpp += f"• {srv} {horario_geral}: {nomes_agrupados}\n"
            
            texto_wpp += "\nBom dia a todos!"
            
            # Exibe a caixa de código pronta para cópia
            st.code(texto_wpp, language="text")

    # -------------------------------------------------------------------------
    # TELA 2: VISÃO PANORÂMICA (CONSULTA MENSAIS)
    # -------------------------------------------------------------------------
        ...
        with st.container(border=True):
            ...
            with col1:
                mes_selecionado = st.selectbox("Selecione o Mês:", options=list(MESES_PT.keys()), format_func=lambda x: MESES_PT[x], index=datetime.now().month - 1)
            with col2:
        ...
        if buscar:
            ...
            if escala_filtrada:
                ...
                st.download_button(
                    ...
                    data=csv_dados,
                    file_name=f"escala_{MESES_PT[mes_selecionado]}_{ano_selecionado}.csv",
                    mime="text/csv",
